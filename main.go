			...
			case *linebot.TextMessage:
				...
				//} else if detail.Ktp != "" {
					if result.Intent == "CLOSINGS"{
						...
						if _, err = bot.ReplyMessage(event.ReplyToken,linebot.NewTemplateMessage("Carousel alt text", handleText(message,event.ReplyToken)), linebot.NewTextMessage(fmt.Sprintf("%s",result.Answer))).Do(); err != nil {
							log.Print(err)
						}
